                ...
                <div class="row">
                    ...
                    <div class="col-sm-12">
                        <div class="form-group">
                            ...
                            <span class="text-danger" id="image_msg" style="display:none;"></span>
                        </div>
                    </div>
                </div>
                <!-- Biography -->
...
<script>
    document.getElementById('doctorForm').addEventListener('submit', function(e) {
        ...
        if (!doctorName || !nameRegex.test(doctorName)) {
            document.getElementById('doctor_name_msg').textContent = "Name must contain only letters and spaces!";
        ...
        if (!phone || !phoneRegex.test(phone)) {
            document.getElementById('phone_msg').textContent = "Phone must be 9-10 digits!";
        ...
        // Image validation
        const imageInput = document.querySelector('.upload-input input[type="file"]');
        const imageMsg = document.getElementById('image_msg');
        if (imageInput && imageInput.files.length > 0) {
            const file = imageInput.files[0];
            const allowedTypes = ['image/jpeg', 'image/png', 'image/gif'];
            if (!allowedTypes.includes(file.type)) {
                imageMsg.textContent = "Only JPG, PNG, or GIF images are allowed!";
                imageMsg.style.display = 'block';
                valid = false;
            } else if (file.size > 2 * 1024 * 1024) { // 2MB
                imageMsg.textContent = "Image size must be less than 2MB!";
                imageMsg.style.display = 'block';
                valid = false;
            } else {
            ...
            imageMsg.textContent = "Please select an image!";
            imageMsg.style.display = 'block';
            valid = false;
        }
